...
public class FabricLanguageProviderMixin {
    @Shadow @Final protected FabricDataOutput dataOutput;

    // For some reason, Fabric API has no way to change the output folder without some weird hacks like this or rewriting a bunch of code.
    ...
    private DataOutput.OutputType forceDataLanguageGeneration(DataOutput.OutputType type) {
        if (!dataOutput.getModId().equals(Waystones.MOD_ID)) return type;

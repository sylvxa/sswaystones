...
public class NameGenerator {
    private static final String[] NAME_FIRST = readResourceAsString("assets/sswaystones/lang/name_first.txt").replace("\r", "").split("\n");
    private static final String[] NAME_LAST = readResourceAsString("assets/sswaystones/lang/name_last.txt").replace("\r", "").split("\n");

    private static String readResourceAsString(String path) {
        try (InputStream stream = NameGenerator.class.getClassLoader().getResourceAsStream(path)) {
            if (stream == null) return "MISSINGNO";
